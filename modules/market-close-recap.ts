...
function getMarketCloseRecapPrompt(date: Date, tickerFacts: MarketCloseTickerFact[]): string {
  ...
  return [
    ...
    "- Risk-on: breite Stärke gleichgerichtet über `ES`, `NQ`, `RTY`, fallender oder ruhiger `VIX`, Risikoappetit.",
    "- Risk-off: breite Schwäche gleichgerichtet über `ES`, `NQ`, `RTY`, steigender `VIX`, Defensive/Absicherung dominiert.",
    "- Cash: wenig Richtung und enge Spanne — alle Indizes etwa ±0,2% oder weniger, ruhiger `VIX`, dünne Signale, abwartender Handel.",
    "- Chaos: starke Reversals, gemischte Vorzeichen zwischen `ES`, `NQ` und `RTY`, hohe Intraday-Spanne im Verhältnis zur Netto-Veränderung oder `VIX`-Sprung; headline-getriebener oder schwer lesbarer Handel.",
    "Nutze `Cash` und `Chaos`, wenn sie besser passen als eine Richtung: `Cash` an ruhigen, richtungslosen Tagen, `Chaos` an wilden Reversal-/Streuungs-Tagen. Nicht jeder Tag ist `Risk-on` oder `Risk-off`.",
