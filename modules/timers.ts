...
export function startNyseTimers(client, channelID: string) {
  ...
  Schedule.scheduleJob(ruleNYSEPremarketOpen, () => {
    if (false === isHoliday(new Date())) {
      ...
      client.channels.cache.get(channelID).send("🛍️🏝️🛥️🥺 Guten Morgen liebe Hebelhelden! Heute bleibt die Börse geschlossen. Genießt den Tag und gebt eure Gewinne für tolle Sachen aus! 🥺🛥️🏝️🛍️").catch(console.error);
    }
  });
