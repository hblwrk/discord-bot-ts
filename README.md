...
echo -n "hunter9" | docker secret create production_hblwrk_trading_calendar_thread_ID -
echo -n "hunter10" | docker secret create production_discord_btcusd_token -
echo -n "hunter11" | docker secret create production_discord_btcusd_client_ID -
echo -n "hunter12" | docker secret create production_gemini_api_key -
...
```json
{
  ...
  "hblwrk_trading_calendar_thread_ID": "",
...
Calendar announcements and calendar reminder alerts post to `hblwrk_trading_calendar_thread_ID` when that thread is configured, and fall back to `hblwrk_channel_OtherAnnouncement_ID` when it is unavailable. Earnings reminders and earnings result announcements post into `hblwrk_channel_OtherAnnouncement_ID`. Scheduled daily and weekly earnings posts send Earnings Whispers most-anticipated tickers in a dedicated message and exclude those tickers from the regular earnings message. Earnings result announcements are sent after a matching SEC EDGAR filing appears. The result watcher includes companies with at least `$100B` market cap and companies on the matching Earnings Whispers weekly X post when that source is available; the daily earnings schedule still places those promoted tickers on their reporting day. Result announcements include an optional three-sentence AI summary and an `Outlook` block when the filing has an explicit outlook or guidance section. Earnings metrics enter as accession-scoped XBRL and filing-HTML candidates with provenance, period, and basis metadata; HTML candidates also carry exact filing evidence. The bot reconciles those candidates before formatting, omits unresolved conflicts, and lets AI select only an existing verified candidate when filing sources disagree. Provider estimates are attached only when the provider's paired actual matches the filing metric, so GAAP EPS, adjusted EPS, and AFFO per share are not compared across bases. Earnings summaries and suspicious-post checks run through the AI provider boundary; `ai_provider` defaults to `gemini`, which runs when `gemini_api_key` is configured. Summary sentences require exact filing snippets and deterministic numeric support before posting. Earnings summaries use bounded opening excerpts plus guidance/outlook windows from the SEC release. MNC announcements use the same optional AI provider access to add a one-minute Discord Markdown summary above the PDF attachment. NYSE pre-market warmups use the same provider boundary to turn the verified session open and fresh `ES`, `NQ`, `RTY`, and `VIX` market-data bot snapshots into a short German Discord message, with deterministic fallback copy when AI output is unavailable or fails validation. NYSE close announcements ground `ES`, `NQ`, `RTY`, and `VIX` facts in the same market-data bot snapshots, fall back to the configured Investing.com market-data pids and Yahoo Finance daily bars when snapshots are unavailable, use optional provider web-search grounding for context, and match the day to the opening sentiment poll; if the bot restarts during the session, it recovers the poll from recent channel history when possible. The provider boundary also supports `ai_provider=openai` with `openai_api_key` and defaults `openai_model` to `gpt-5.4-mini`. Gemini local call caps default to `9` per minute and `18` per day through `gemini_calls_per_minute` and `gemini_calls_per_day`; OpenAI local call caps default to `20` per minute and `200` per day through `openai_calls_per_minute` and `openai_calls_per_day`. Deployments with a higher Gemini quota set `production_gemini_calls_per_minute` and `production_gemini_calls_per_day` to raise those local caps. Calendar and earnings reminders ping the `alerts` special role, which is self-assignable from the roles channel via the `🛎️` bellhop bell reaction on the special roles message.
...
Real-time market-data is pulled in through websocket connections and distributed via Discord bot nickname and presence information. Those bots can be joined to the server separately and their runtime information is managed as an asset. They require no oAuth2 scopes other than "bot". Crypto market-data assets with a `tastytradeStreamerSymbol` use tastytrade quote streaming first and fall back to the existing Investing.com stream with capped exponential retry backoff; crypto assets without that field use Investing.com only. For tastytrade crypto assets, configure the visible crypto symbol such as `BTC/USD`; the runtime resolves it to the tastytrade streamer symbol before subscribing.
